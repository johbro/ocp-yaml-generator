...
<template>
  <div>
    <Field label="Name"><input type="text" v-model="iface.name" placeholder="eno1, bond0, vlan100, br0" /></Field>
    <Field label="State">
      <select v-model="iface.state">
        <option v-for="o in ifaceStateOptions" :key="o" :value="o">{{ o }}</option>
      </select>
    </Field>
    <Field label="MTU"><input type="number" v-model.number="iface.mtu" placeholder="1500" /></Field>
    <Field v-if="iface.type === 'ethernet' || iface.type === 'bond'" label="MAC address">
    ...
    <div v-if="iface.type === 'bond'" class="card">
      ...
      <Field label="Ports" help="Slave interfaces. Define ethernet ports above first.">
        <div class="tag-input">
    ...
    <div v-if="iface.type === 'vlan'" class="card">
      ...
      <Field label="VLAN ID"><input type="number" min="1" max="4094" v-model.number="iface.vlan.id" /></Field>
    </div>

    <div v-if="iface.type === 'linux-bridge'" class="card">
      <header><span class="title">Linux bridge</span></header>
      <Field label="STP enabled">
    ...
    <div class="card">
      ...
      <Field label="Enabled"><input type="checkbox" v-model="iface.ipv4.enabled" /></Field>
      <template v-if="iface.ipv4.enabled">
        <Field label="DHCP"><input type="checkbox" v-model="iface.ipv4.dhcp" /></Field>
        <Field v-if="!iface.ipv4.dhcp" label="Static addresses">
    ...
    <div class="card">
      ...
      <Field label="Enabled"><input type="checkbox" v-model="iface.ipv6.enabled" /></Field>
      <template v-if="iface.ipv6.enabled">
        <Field label="DHCP"><input type="checkbox" v-model="iface.ipv6.dhcp" /></Field>
        <Field label="Autoconf (SLAAC)"><input type="checkbox" v-model="iface.ipv6.autoconf" /></Field>
        <Field v-if="!iface.ipv6.dhcp && !iface.ipv6.autoconf" label="Static addresses">
